...
source("~/Desktop/Indicator/lbl.R")
periFT=0
areaFT=0
...
uniq=unique(as.vector(a)); #uniq= unique(C.Pr);
uniq=uniq[uniq>0];
for (i in uniq){
...
write.csv(cbind(patchindex, periFT, areaFT),"permiter-area.csv")
ggplot(areaFT~periFT,log="xy",pch=16)
output=list(areaFT,periFT)
}
